...
function Vendor() {
	...
	const [ vendorLocations, setVendorLocations ] = useState([{ name: "", streetAddress: "", city: "", state: "", zipCode: 28078, daysAtLocation: "", hoursAtLocation: ""}]);
	const [ locationCount, setLocationCount ] = useState(1);
	const [ allVendorProducts, setAllVendorProducts ] = useState([{name: "", isFeatured: false}]);
  ...
  const helpSetProductsFeatured = (numVal) => {
    ...
    <Navigation />
		<main className="body animated slideInLeft">
			...
			<form onSubmit={(e) => handleSubmit(e)} className="text-center border border-light p-5" action="#!">
          ...
          <div className="form-group border border-primary rounded">
            ...
            }} className="form-control my-4" placeholder="Market Name" type="text" />
            <input onChange={e => {
              const newArray = vendorLocations
              newArray[0].streetAddress = e.target.value
              setVendorLocations(newArray)
            }} className="form-control my-4" placeholder="Street Address" type="text" />
            <input onChange={e => {
              const newArray = vendorLocations
              newArray[0].city = e.target.value
              setVendorLocations(newArray)
            }} className="form-control mb-4" placeholder="City" type="text" />
            <input onChange={e => {
              const newArray = vendorLocations
            ...
            <input onChange={e => {
              ...
              setVendorLocations(newArray)
            }} className="form-control mb-4" placeholder="Zip Code" type="number" />
            <input onChange={e => {
              const newArray = vendorLocations
              newArray[0].daysAtLocation = e.target.value
              setVendorLocations(newArray)
            }} className="form-control mb-4" placeholder="Days At Location" type="text" />
            <input onChange={e => {
              const newArray = vendorLocations
              newArray[0].hoursAtLocation = e.target.value
              setVendorLocations(newArray)
            }} className="form-control mb-4" placeholder="Hours At Location" type="text" />
					</div>
				...
				<div className="form-group">
					...
					<button onClick={(e) => setProductCount(productCount + 1)} className="btn btn-primary m-2">
						Add Product
					</button>
					<button onClick={(e) => setProductCount(productCount - 1)} className="btn btn-secondary m-2">
						Subtract Product
					</button>
					{locationCount > 0 ? (
						createProductDivs().map((val) => {
							return (
								<div className="form-group border border-primary rounded">
									<input id={val} onChange={e => helpSetProducts(e.target.value, e.target.id)} className="form-control my-4" placeholder="Product Name" type="text" />
